        ...
        match bytes[i] {
            b'"' => {
                ...
                if k < bytes.len() && bytes[k] == b':' {
                    ...
                    let key: String = serde_json::from_str(&escaped).map_err(|_| "invalid-key".to_string())?;
...
fn walk(value: &Value) -> Result<(), String> {
    match value {
        Value::Number(n) => {
            if let Some(v)=n.as_i64() { if v.unsigned_abs() > MAX_EXACT_INT as u64 { return Err("integer-range".into()); } }
...
fn canonical(value: &Value) -> Result<Vec<u8>, String> {
    match value {
        ...
        Value::Number(number) => {
            let value = number.as_i64().ok_or_else(|| "fractional-number-profile".to_string())?;
            if value.unsigned_abs() > MAX_EXACT_INT as u64 { return Err("integer-range".into()); }
            Ok(value.to_string().into_bytes())
        },
        Value::String(string) => serde_json::to_string(string).map(|x| x.into_bytes()).map_err(|e| e.to_string()),
        Value::Array(values) => {
        ...
        Value::Object(values) => {
            ...
            for (index, (key, child)) in entries.iter().enumerate() {
                if index > 0 { output.push(b','); }
                output.extend(serde_json::to_string(key).map_err(|e| e.to_string())?.as_bytes());
                output.push(b':');
                output.extend(canonical(child)?);
            }
            output.push(b'}');
...
fn serve(path:&Path)->Result<(),String>{let _=fs::remove_file(path);let listener=UnixListener::bind(path).map_err(|e|e.to_string())?;let(mut stream,_)=listener.accept().map_err(|e|e.to_string())?;let mut seen=HashSet::new();let mut queue:Vec<String>=Vec::new();loop{let raw=match recv_frame(&mut stream){Ok(raw)=>raw,Err(error)if error=="short-header"=>break,Err(error)=>{send_frame(&mut stream,&serde_json::json!({"accepted":false,"reason":error}))?;continue}};let result=if queue.len()>=2{serde_json::json!({"accepted":false,"reason":"queue-full","queue_depth":queue.len()})}else{queue.push(raw.clone());let mut value=process(&raw,&mut seen).unwrap_or_else(|e|serde_json::json!({"accepted":false,"reason":e}));value["queue_depth"]=serde_json::json!(queue.len());queue.remove(0);value};let mut value=result;value["log"]=serde_json::json!("qualification-shell;payload-minimized");send_frame(&mut stream,&value)?;}drop(listener);let _=fs::remove_file(path);Ok(())}

fn self_test(fixture:&Path)->Result<(),String>{let raw=fs::read_to_string(fixture).map_err(|e|e.to_string())?;let first=process(&raw,&mut HashSet::new())?;let mut seen=HashSet::from(["message-0001".to_string()]);let again=process(&raw,&mut seen)?;if first["fixed_point_state"]!=25 || again["fixed_point_state"]!=0{return Err("idempotency".into())};for bad in [r#"{"message_id":"a","message_id":"b"}"#,r#"{"a":1,"\u0061":2}"#,r#"{"x":NaN}"#,r#""\ud800""#]{if parse(bad).is_ok(){return Err("invalid-accepted".into())}};let mut event=parse(&raw)?;event.insert("schema_version".into(),Value::String("2.0".into()));if validate(&event).is_ok(){return Err("major-accepted".into())};let mut queue=Vec::new();queue.push("one");queue.push("two");if queue.len()!=2{return Err("queue-probe".into())};println!("{}",serde_json::json!({"self_test":"passed","digest":first["digest"],"rust":env!("CARGO_PKG_VERSION"),"queue_probe":"passed"}));Ok(())}
fn main(){let a:Vec<String>=env::args().collect();let result=if a.len()==3&&a[1]=="--self-test"{self_test(Path::new(&a[2]))}else if a.len()==3&&a[1]=="--serve"{serve(Path::new(&a[2]))}else if a.len()==3&&a[1]=="--canonical"{fs::read_to_string(&a[2]).map_err(|e|e.to_string()).and_then(|raw|parse(&raw)).and_then(|value|canonical(&Value::Object(value))).map(|bytes|println!("{}",base64_encode(&bytes)))}else{Err("usage: --self-test FIXTURE | --serve SOCKET | --canonical FILE".into())};if let Err(e)=result{eprintln!("{e}");std::process::exit(2)}}

fn base64_encode(bytes:&[u8])->String{const TABLE:&[u8]=b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";let mut out=String::new();let mut i=0;while i<bytes.len(){let a=bytes[i];let b=if i+1<bytes.len(){bytes[i+1]}else{0};let c=if i+2<bytes.len(){bytes[i+2]}else{0};out.push(TABLE[(a>>2)as usize]as char);out.push(TABLE[(((a&3)<<4)|(b>>4))as usize]as char);if i+1<bytes.len(){out.push(TABLE[(((b&15)<<2)|(c>>6))as usize]as char)}else{out.push('=')}if i+2<bytes.len(){out.push(TABLE[(c&63)as usize]as char)}else{out.push('=')}i+=3;}out}
