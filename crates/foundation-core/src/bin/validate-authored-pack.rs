use foundation_core::contracts::MonIngestedFramePackV1;
...
fn run() -> Result<(), String> {
    ...
    let pack: MonIngestedFramePackV1 = serde_json::from_slice(&bytes)
        .map_err(|error| format!("generated pack invalid: {error}"))?;
    if pack.profile != "MON_INGESTED_FRAME_PACK_V1"
        || pack.schema_version != 1
        || pack.source_profile != "MON_AUTHORED_FRAME_SOURCE_PACK_V1"
    {
        return Err("unexpected ingested pack profile or schema version".to_owned());
    }
    ...
    if pack.tracks.is_empty()
        || pack.tracks.iter().any(|track| track.frames.is_empty())
        || pack.source_assets.is_empty()
    {
    ...
    let decoded: MonIngestedFramePackV1 = serde_json::from_slice(&round_trip)
