...
func accept_intent(candidate: Dictionary) -> Dictionary:
	...
	if sequence_value == null or int(sequence_value) != sequence_value or int(sequence_value) < 0:
...
func tick_once() -> Dictionary:
	...
	semantic_tick += 1
	if velocity != 0:
		_animation_phase = fmod(_animation_phase + _presentation_rate(velocity) / SEMANTIC_HZ, 1.0)
	if actor_root != null:
